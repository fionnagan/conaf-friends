...
// Committed denylist for raw "guest" entries that aren't an individual
// person at all — a recurring bit/segment title (Conan Scrapisode, DIY
// Conan), a show/franchise credited as if it were the guest (The Walking
// Dead, Ghost Hunters), a group (Denver Broncos Offensive Line), or an
// episode title that got parsed as a guest name (a memorial/tribute
// episode with no single bookable guest). No Wikipedia entity resolution
// will ever find these — confirmed by hand against the current
// needs_review backlog, not guessed at. Applied here (merge time, both
// raw sources) rather than deleted from data/guests.json directly, since
// that file is regenerated by weekly-ingest.yml and a direct edit would
// silently revert on the next run.
function loadExcludedGuestNames(): Set<string> {
  try {
    const p = path.join(__dirname, 'excluded-guest-names.json');
    const names: string[] = JSON.parse(fs.readFileSync(p, 'utf8'));
    return new Set(names.map(normalizeGuestName));
  } catch {
    return new Set();
  }
}
...
export function merge(
  ...
  const excludedGuestNames = loadExcludedGuestNames();
  ...
  for (const ep of podcastEpisodes) {
    if (ep.guestName && !ep.isFanSegment && !ep.isStaffEpisode) {
      ...
      if (!excludedGuestNames.has(name)) allGuestNames.add(name);
    }
  }
  ...
  for (const ep of podcastEpisodes) {
    ...
    if (excludedGuestNames.has(normalizeGuestName(ep.guestName))) continue;
  ...
  for (const ln of lateNightHistory) {
    const name = normalizeGuestName(ln.guestName);
    // Skipped before touching episodeSeq, so a real guest's billing-order
    // index reflects only actual guests in that episode, not an excluded
    // segment/show-title entry that happened to sit ahead of them.
    if (excludedGuestNames.has(name)) continue;
    const guest = getGuest(name);
